...
function update_rs!(rs, r, denom, iris_factor)
    update_rsquared2!(rs, r)
    @turbo for k ∈ 1:3
        rs[2,k] = sqrt(exp(-rs[1,k]/denom)) # sqrt from √s
    end
    if isfinite(iris_factor)
        iris_r_sq = iris_factor^2 * 2 * denom
        @inbounds @fastmath for k ∈ 1:3
            if rs[1,k] > iris_r_sq
                rs[2,k] = zero(eltype(rs))
            end
        end
    end
    return nothing
...
function update_fields_fast!(p, r, t)
    ...
    update_rs!(p.rs, r, p.denom, p.iris_factor)
